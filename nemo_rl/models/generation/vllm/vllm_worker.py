...
class BaseVllmGenerationWorker:
    ...
    def _build_sampling_params(
        ...
        *,
        ...
        if self.cfg.get("ignore_eos_token", False):
            sampling_kwargs["ignore_eos"] = True

        return self._create_sampling_params(sampling_kwargs)

    def _create_sampling_params(self, sampling_kwargs: dict[str, Any]):
        try:
            return self.SamplingParams(**sampling_kwargs)
        except TypeError:
            if "ignore_eos" not in sampling_kwargs:
                raise
            sampling_kwargs = dict(sampling_kwargs)
            sampling_kwargs.pop("ignore_eos")
            return self.SamplingParams(**sampling_kwargs)
