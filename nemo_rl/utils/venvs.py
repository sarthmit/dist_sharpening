...
def _prepend_include_paths(env: dict[str, str], include_roots: list[str]) -> None:
    """Ensure compilers can find system multiarch headers."""
    existing = env.get("CPATH", "")
    env["CPATH"] = ":".join(include_roots + ([existing] if existing else []))

    existing = env.get("C_INCLUDE_PATH", "")
    env["C_INCLUDE_PATH"] = ":".join(include_roots + ([existing] if existing else []))

    existing = env.get("CPLUS_INCLUDE_PATH", "")
    env["CPLUS_INCLUDE_PATH"] = ":".join(include_roots + ([existing] if existing else []))
...
def create_local_venv(
    ...
    _prepend_include_paths(
        env,
        ["/usr/include", "/usr/include/x86_64-linux-gnu"],
    )
    # Keep compile and link toolchains consistent in mixed module environments.
    env.setdefault("CC", "gcc")
    env.setdefault("CXX", "g++")
    # Some CUDA extension builders try to inspect a live GPU when this is unset.
    # Set a conservative default so worker venv builds can run in CPU-only setup actors.
    env.setdefault("TORCH_CUDA_ARCH_LIST", "9.0")
    # mamba-ssm pulls causal-conv1d, but DTensor transformer training paths do not require
    # its CUDA extension. Skipping it avoids long/fragile nvcc builds on setup actors.
    env.setdefault("CAUSAL_CONV1D_SKIP_CUDA_BUILD", "TRUE")
