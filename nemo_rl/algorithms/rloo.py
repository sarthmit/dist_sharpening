...
class AdvEstimatorConfig(TypedDict):
    """Configuration for advantage estimator (GRPO, leave-one-out, or Reinforce++)."""

    name: str  # "grpo", "leave_one_out", or "reinforce_plus_plus"
    # GRPO specific
    normalize_rewards: NotRequired[bool]
    use_leave_one_out_baseline: NotRequired[bool]
    # Leave-one-out specific
    # normalize_rewards is shared with GRPO
    # Reinforce++ specific
...
class GRPOConfig(TypedDict):
    ...
    # Advantage estimator configuration (grpo, leave_one_out, or reinforce_plus_plus)
...
class DistSharpeningReward:
    """Reward transformer for distributional sharpening."""
    ...
    def __init__(
        ...
        self.weight = weight
        self.norm_fn = self._resolve_norm_fn()

        if self.mode not in {"none", "optimization", "sampling"}:
            raise ValueError(f"Invalid dist sharpening mode: {self.mode}")

    def _resolve_norm_fn(self):
        if self.normalization == "none":
            return None
        if self.normalization == "sum":
            return lambda values, mask: masked_sum(values, mask, dim=1)
        if self.normalization == "mean":
            return lambda values, mask: masked_weighted_mean(
                values, mask, dim=1, alpha=1.0
            )
        if self.normalization == "weighted_mean":
            return lambda values, mask: masked_weighted_mean(
                values, mask, dim=1, alpha=self.weight
            )
        raise ValueError(
            f"Invalid dist sharpening normalization: {self.normalization}"
        )

    def _norm_ref(self, ref_logprobs: torch.Tensor, token_mask: torch.Tensor):
        if self.norm_fn is None:
            return torch.zeros(
                ref_logprobs.shape[0],
                device=ref_logprobs.device,
                dtype=ref_logprobs.dtype,
            )
        return self.norm_fn(ref_logprobs, token_mask)

    def __call__(
        self,
        rewards: torch.Tensor,
        logprobs_policy: torch.Tensor | None,
        logprobs_reference: torch.Tensor | None,
        token_mask: torch.Tensor | None,
    ) -> torch.Tensor:
        if self.mode == "none":
            return rewards
        if logprobs_reference is None or logprobs_policy is None or token_mask is None:
            raise ValueError(
                "DistSharpeningReward requires policy/ref logprobs and token_mask."
            )

        ref_sum = masked_sum(logprobs_reference, token_mask, dim=1)
        policy_sum = masked_sum(logprobs_policy.detach(), token_mask, dim=1)
        norm_ref = self._norm_ref(logprobs_reference, token_mask)

        if self.mode == "optimization":
            if self.beta_inv == 0.0:
                return self.alpha * norm_ref
            return rewards + (self.alpha / self.beta_inv) * norm_ref

        if self.beta_inv == 0.0:
            return (self.alpha - 1.0) * norm_ref + ref_sum - policy_sum
        return rewards + ((self.alpha - 1.0) / self.beta_inv) * norm_ref + (
            1.0 / self.beta_inv
        ) * (ref_sum - policy_sum)
...
def _create_advantage_estimator(master_config: MasterConfig):
    ...
    Returns:
        An advantage estimator instance (GRPOAdvantageEstimator, LeaveOneOutAdvantageEstimator, or ReinforcePlusPlusAdvantageEstimator).
    ...
    elif adv_estimator_name == "leave_one_out":
        adv_estimator = LeaveOneOutAdvantageEstimator(
            adv_estimator_config, loss_config
        )
        print("  ✓ Using leave-one-out advantage estimator")
        ...
        for batch in dataloader:
            ...
            with timer.time("total_step_time"):
                ...
                with timer.time("advantage_calculation"):
                    ...
                    # Log advantages information
                    logger.log_histogram(
                        train_data["advantages"].numpy(),
                        total_steps + 1,
                        "train/advantages/histogram",
                    )
                    metrics["advantages/sum"] = (
                        train_data["advantages"].float().sum().item()
                    )
                    metrics["advantages/mean"] = (
                        train_data["advantages"].float().mean().item()
                    )
