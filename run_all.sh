...
# for runner in run_qwen_3b.sh run_llama_3b.sh; do
  # sbatch --account=aip-glaj "$runner" 1.0 0.0 optimization sum 1.0
  # sbatch --account=aip-glaj "$runner" 1.0 0.0 optimization weighted_mean 1.0
  ...
  # for beta_inv in 0.0 100.0 10000.0; do
    ...
    # sbatch --account=aip-glaj "$runner" 4.0 "$beta_inv" sampling weighted_mean 1.0

  #   sbatch --account=aip-glaj "$runner" 2.0 "$beta_inv" sampling sum 0.0
    # sbatch --account=aip-glaj "$runner" 2.0 "$beta_inv" sampling weighted_mean 0.5
...
# for runner in run_qwen3_4b_deepscaler.sh; do
  ...
  # sbatch --account=aip-glaj "$runner" 4096 1.0 0.0 optimization weighted_mean 1.0
  # sbatch "$runner" 1.0 0.0 optimization mean 0.0
  # sbatch --account=aip-bengioy "$runner" 4096 0.0 1.0 optimization sum 0.0

  # for beta_inv in 100.0; do
...
for runner in run_qwen3_reasoning_gym_sharpening.sh; do
 sbatch --account=aip-glaj "$runner" 8192 1.0 0.0 optimization sum 0.0
 sbatch --account=aip-glaj "$runner" 8192 0.0 1.0 optimization sum 0.0
 # sbatch "$runner" 1.0 0.0 optimization weighted_mean 0.5
 # sbatch "$runner" 1.0 0.0 optimization mean 0.0

 for beta_inv in 0.0 100.0 10000.0; do
   sbatch --account=aip-glaj "$runner" 8192 4.0 "$beta_inv" sampling sum 0.0
   # sbatch "$runner" 4.0 "$beta_inv" sampling weighted_mean 0.5

   sbatch --account=aip-glaj "$runner" 8192 2.0 "$beta_inv" sampling sum 0.0
   # sbatch "$runner" 2.0 "$beta_inv" sampling weighted_mean 0.5

   sbatch --account=aip-glaj "$runner" 8192 1.0 "$beta_inv" sampling sum 0.0

   sbatch "$runner" 8192 1.25 "$beta_inv" sampling sum 0.0
   # sbatch "$runner" 1.25 "$beta_inv" sampling weighted_mean 0.5
