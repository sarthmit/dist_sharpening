...
def test_reinforce_plus_plus_leave_one_out_baseline():
    """Test that ReinforcePlusPlusAdvantageEstimator can use leave-one-out baseline."""
    estimator_config = {
        "minus_baseline": True,
    ...
    rewards = torch.tensor([0.0, 1.0, 4.0, 6.0])
    mask = torch.ones(4, 3)
    ...
    expected = (rewards - baseline).unsqueeze(-1).expand(mask.shape)
    expected_mean = (expected * mask).sum() / mask.sum()
    expected_var = ((expected - expected_mean).pow(2) * mask).sum() / mask.sum()
    expected_rstd = expected_var.clamp(min=1e-8).rsqrt()
    expected = (expected - expected_mean) * expected_rstd
