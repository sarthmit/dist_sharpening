...
from nemo_rl.algorithms.rloo import MasterConfig, grpo_train, setup
...
def main() -> None:
    ...
    # Keep W&B run name aligned with log/checkpoint run directory naming.
    log_run_name = os.path.basename(os.path.normpath(config["logger"]["log_dir"]))
    if config["checkpointing"]["enabled"]:
        checkpoint_run_name = os.path.basename(
            os.path.normpath(config["checkpointing"]["checkpoint_dir"])
        )
        if checkpoint_run_name != log_run_name:
            print(
                "⚠️ Log/checkpoint run names differ; using log directory name "
                f"for W&B: {log_run_name}"
            )
    config["logger"].setdefault("wandb", {})
    config["logger"]["wandb"]["name"] = log_run_name
    ...
    print(f"📊 Using W&B run name: {config['logger']['wandb']['name']}")
